...
/* ============================================================
    ...
    try {
      ...
      blocks.forEach(function (block, idx) {
        ...
        mermaid.render(id, code.trim()).then(function (result) {
          ...
          /* keep data-diagram so reinitMermaid can re-render on theme change */
          mermaidReady = true;
  ...
  /* Mermaid content is already in data-diagram attribute (set by generate_site.py).
     Nothing to prepare — blocks start empty, diagram code is in the attribute. */

  /* Expose for dark mode toggle in chunk 4 */
  window._RT.reinitMermaid = function (theme) {
    /* data-diagram attribute is always preserved — just clear rendered SVG */
    document.querySelectorAll('.mermaid').forEach(function (block) {
      block.innerHTML = '';
    });
    initMermaid(theme);
...
/* ============================================================
  ...
  function applyTheme(theme) {
    ...
    /* re-init mermaid only after it has already rendered once */
    if (mermaidReady) {
